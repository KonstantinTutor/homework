...
print(len(set(nums)))
# print(sum([1 for i in nums if nums.count(i) == 1]))  Вот здесь обратите внимание. Вы считаете кол-во чисел, которые встречаются только 1 раз. Но надо было чуть больше
# Надо было просто удалить все копии из списка и вернуть длину такого списка. Например: если в списке два числа 13, то ваш код проигнорирует оба эти числа.
# А правильный код должен посчитать +1 к общей сумме за каждое уникальное число. То есть и за оба числа 13 программа должна бы добавить +1 к ответу.
